    ...
    module VnodeCommandReceiver
      ...
      def ev_spushv(s)
        ...
        if @stats.spushv_protection
          @loggerger.info("#{__method__}:In spushv_protection")
        ...
        count = rcount = 0
        @logger.debug("#{__method__}:#{s.inspect} received.")
        loop {
          ...
          if vlen != 0
            if klen > @stats.spushv_klength_warn
              @logger.warn("#{__method__}:Too long key: key = #{k}")
            end
            if vlen > @stats.spushv_vlength_warn
              @logger.warn("#{__method__}:Too long value: key = #{k} vlen = #{vlen}")
            end
...
#              @logger.debug("#{__method__}:[#{vn} #{last} #{clk} #{expt} #{k}] was stored.")
              ...
              rcount += 1
#              @logger.warn("#{__method__}:[#{vn} #{last} #{clk} #{expt} #{k}] was rejected.")
            end
          else
            createhash(s[1]) unless @storages[s[1]]
...
#              @logger.warn("#{__method__}:[#{vn} #{last} #{clk} #{expt} #{k}] was rejected.")
        ...
        @logger.debug("#{__method__}:#{s[1]}_#{s[2]} #{count} keys loaded.")
        @logger.debug("#{__method__}:#{s[1]}_#{s[2]} #{rcount} keys rejected.") if rcount > 0
        ...
        if Config.const_defined?(:STORAGE_EXCEPTION_ACTION) &&
          ...
          @logger.error("#{__method__}:Romad will be stop.")
      ...
      def ev_reqpushv(s)
        ...
        if(@stats.run_iterate_storage || @stats.run_join || @stats.run_balance)
          @logger.warn("reqpushv rejected:#{s}")
          send_data("REJECTED\r\n")
          return
        end
